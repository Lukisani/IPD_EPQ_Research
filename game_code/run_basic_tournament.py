...
from files import*
# from axelrod_strategies_1 import*

direc = ObjectView(get_direc())


# axelrod_players = [TitForTat(), TidemanChieruzzi(), Nydegger(), Grofman(), Shubik(),
#     SteinRapoport(), Grudger(), Davis(), Graaskamp(), Downing(),
#     Feld(), Joss(), Tullock(), Anonymous(), RandomPlayer()]
...
def average_basic_tournament(rounds=100, average=10, reward=3, temptation=5, sucker=0, punishment=1, custom=False):
    list_of_results = []
    list_of_scores = []
    ...
    # Custom file name for custom experiments

    if custom:
        # Saves results all under one folder
        new_folder_name = 'axelrod1.3'
